...
for file in os.listdir(synthetic_images):
    ...
    dst_img = os.path.join(final_images,f"synthetic_{file}")
    dst_mask = os.path.join(final_masks,f"synthetic_{file.replace('.tif','.png')}")
...
for file in os.listdir(weak_images):
    ...
    dst_img = os.path.join(final_images,f"weak_{file}")
    dst_mask = os.path.join(final_masks,f"weak_{file.replace('.tif','.png')}")
...
print("Total files merged:", count)
